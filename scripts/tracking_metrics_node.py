...
class TrackingMetrics(Node):
    def __init__(self) -> None:
        ...
        # --- Params (relative defaults -> namespace dostu) ---
        ...
        self.error_topic: str = str(self.declare_parameter("error_topic", "metrics/error").value)
        self.rmse_topic: str = str(self.declare_parameter("rmse_topic", "metrics/rmse").value)
        self.rmse_window_topic: str = str(self.declare_parameter("rmse_window_topic", "metrics/rmse_window").value)

        self.rmse_window_N: int = int(self.declare_parameter("rmse_window_N", 200).value)

        # Reset service (ayrı)
        self.reset_srv: str = str(self.declare_parameter("reset_srv", "reset_metrics").value)
        ...
        self.get_logger().info(
            ...
            f"pub(error)={self.error_topic} winN={self._win.maxlen}"
        )
    ...
    def _on_gt(self, msg: Odometry) -> None:
        self._gt_xy = (float(msg.pose.pose.position.x), float(msg.pose.pose.position.y))

    def _on_est(self, msg: Odometry) -> None:
        if self._gt_xy is None:
            return

        ex = float(msg.pose.pose.position.x) - self._gt_xy[0]
        ey = float(msg.pose.pose.position.y) - self._gt_xy[1]
        e = math.hypot(ex, ey)
        e2 = e * e
