...
def generate_launch_description():
    # ---- Launch args (senin terminal komutlarındaki parametrelerin aynısı) ----
    ...
    init_from_gt_arg = DeclareLaunchArgument(
        "init_from_gt", default_value="true",
        description="EKF init_from_gt (true/false)"
    )

    gz_world_arg = DeclareLaunchArgument(
        "gz_world", default_value="empty_world",
        description="Gazebo world name (SetEntityPose service yolu için). Senin servisinde empty_world."
    )
    gz_entity_arg = DeclareLaunchArgument(
        "gz_entity", default_value="ekf_proxy",
        description="Gazebo'da EKF proxy model adı (ekf_tracker_from_range.py bunu set_pose ile sürüyor)"
    )
    ...
    def launch_setup(context, *args, **kwargs):
        ...
        # --- Include: Controller (Gazebo biraz ayağa kalksın diye 3 sn geciktiriyoruz) ---
        ...
        # Node adını özellikle farklı veriyoruz: senin logtaki "/ros_gz_bridge" çakışma uyarısı bitsin.
        ...
        # --- Script paths (senin terminalde python3 ile çalıştırdıkların) ---
        ...
        # --- Marker viz (sensors + GT + EKF + trails) ---
        viz = ExecuteProcess(
            cmd=[
        ...
        # Gazebo başladıktan sonra bridge + node’ları başlatmak genelde daha stabil:
        # (çok şart değil ama “tek launch”ta race condition ihtimalini azaltır)
